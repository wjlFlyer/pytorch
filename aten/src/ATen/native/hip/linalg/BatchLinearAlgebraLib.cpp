...
void apply_ldl_solve_cusolver(
  ...
  TORCH_CUSOLVER_CHECK(hipsolverDnXsytrs_bufferSize(
      handle,
      uplo,
      n,
      nrhs,
  ...
  for (const auto i : c10::irange(batch_size)) {
    ...
    TORCH_CUSOLVER_CHECK(hipsolverDnXsytrs(
        handle,
        uplo,
        n,
        nrhs,
...
static void apply_syevj_batched(const Tensor& values, const Tensor& vectors, const Tensor& infos, bool upper, bool compute_eigenvectors) {
  ...
  TORCH_CUSOLVER_CHECK(hipsolverDnCreateParams(&syev_params));
  ...
  at::cuda::solver::xsyevBatched<scalar_t>(
      ...
      infos_data,
      batch_size);
  TORCH_CUSOLVER_CHECK(hipsolverDnDestroyParams(syev_params));
